...
GameStates.makeGame = function( game, shared ) {
	...
	function fireGreens(){
		...
		var green = greens.getFirstExists(false);

    	if (green)
    	{
    	    green.frame = game.rnd.integerInRange(0,6);
    	    green.exists = true;
			green.scale.set(0.5);
    	    green.reset(800, game.world.randomY);
			//game.physics.enable(ob, Phaser.Physics.ARCADE);
			green.body.velocity.x = -350;
            green.body.allowGravity = 0;
    	    //star.body.bounce.y = 0.8;
			//obSpeed -= 2;
    	}
	}
	...
	function greenHandler(sprite, green){
		green.kill();
		percent += 2;
	}
    ...
    return {
        ...
        create: function () {
			...
			obs2 = game.add.group();
			obs2.createMultiple(20, 'redthing', 0, false);
			game.time.events.loop(700, fireObs2, this);

			//GREENS
			greens = game.add.group();
			greens.createMultiple(100,'green',0,false);
			game.time.events.loop(1500, fireGreens, this);
			...
			//PLAYER CONTROLS
			if(firedShot == 0)
				player = game.add.sprite(400,200, 'gold');
			else
        ...
        update: function () {
			...
			game.physics.arcade.overlap(player,greens,greenHandler);
			game.physics.arcade.overlap(redField,greens,greenHandler);
			...
			if(increaseTimer < game.time.now && (!(GO)))
			{
				percent++;
				increaseTimer = game.time.now + 5000;
			}
			...
			greens.forEachAlive(checkBounds, this);
		...
		render: function () {
			...
			game.debug.text('Percent: ' + percent, 32, 64);
			game.debug.text('TIME: ' + time + ' ms', 32, 96);
		}
    };
};
